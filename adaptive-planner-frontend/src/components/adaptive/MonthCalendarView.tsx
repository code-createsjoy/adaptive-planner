...
import { BlockCategory, EnergyLevel, TimeBlock } from '@/types/planner';
import {
  ChevronLeft,
  ChevronRight,
...
export const MonthCalendarView: React.FC<MonthCalendarViewProps> = ({ onSelectDate }) => {
  ...
  // State for Day Detail Popup Modal
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Lịch tháng trực quan · Bấm vào ngày bất kỳ để xem thời khóa biểu & thêm lịch đi chơi/công việc
              </p>
            </div>
          </div>
          ...
          {Array.from({ length: totalDays }).map((_, idx) => {
            ...
            return (
              <button
                ...
                className={`group relative min-h-[90px] sm:min-h-[110px] p-2.5 rounded-xl border text-left flex flex-col justify-between transition-all duration-200 cursor-pointer hover:shadow-md ${
                  isSelected
                ...
                <div className="flex items-start justify-between w-full">
                  ...
                  {isHoliday && (
                    <span
                      ...
                      title={holidayName}
                    >
                      🇻🇳 <span className="hidden xl:inline truncate max-w-[70px]">{holidayName}</span>
                    </span>
                  )}
                </div>
                          ...
                          {Array.from({ length: Math.min(totalTasks, 4) }).map((_, dotIdx) => (
                            <span
                              ...
                              className={`w-1.5 h-1.5 rounded-full ${
                                isHoliday ? 'bg-rose-500' : 'bg-primary'
                              }`}
                            />
                          ))}
                          {totalTasks > 4 && (
...
const DayDetailPopupModal: React.FC<DayDetailPopupModalProps> = ({
  ...
  const handleCreateCustomBlock = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTitle.trim() || !isTimeOrderValid) return;
        ...
        <div className="p-5 border-b border-border flex items-center justify-between bg-muted/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Xem chi tiết thời khóa biểu & thêm khung giờ riêng cho ngày này
              </p>
            </div>
          </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-8 px-4 border border-dashed border-border/80 rounded-2xl bg-muted/10 space-y-2">
                ...
                <p className="text-xs font-bold text-foreground">
                  Ngày này hiện đang trống lịch hoàn toàn!
                </p>
                <p className="text-[11px] text-muted-foreground max-w-sm mx-auto">
                  Bạn có thể chọn một trong các gợi ý phía trên để thêm lịch đi chơi, cà phê hoặc tự tạo khung giờ của bạn.
                </p>
              </div>
            ) : (
